...
function App() {
  ...
  const [unitList2, setUnitList2] = useState<Values>(initVal);

  const initWeaponValues: WeaponData[] = [];
  const initWeaponVal: WeaponValues = {
    data: initWeaponValues,
  };
  const [weaponList, setWeaponList] = useState<WeaponValues>(initWeaponVal);
  ...
  for (let i = 0; i < unitList2.data.length - 1; i++) {
    const initWeaponData: WeaponData[] = [];
    unitList2.data[i].weaponList = initWeaponData;
    for (let j = 0; j < weaponList.data.length - 1; j++) {
      console.log(unitList2);

      if (unitList2.data[i].name === weaponList.data[j].unit) {
        console.log("yay", unitList2.data[i].name);
        unitList2.data[i].weaponList.push(weaponList.data[j]);
      }
    }
  }
